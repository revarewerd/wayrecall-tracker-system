...
> **Обновлено:** 3 марта 2026 | Версия: 2.1
...
| `geozone-events` | 6 | 30 дней | **NEW v2.1** События геозон (въезд/выезд). Producer: Rule Checker |
| `rule-violations` | 6 | 30 дней | **NEW v2.1** Нарушения правил (скорость, геозоны). Producer: Rule Checker |
...
> ℹ️ **Block 2 output топики** (geozone-events, rule-violations) не включены в матрицу выше для читаемости.
> - `geozone-events`: **W** = Rule Checker, **R** = Notification Service, WebSocket Service
> - `rule-violations`: **W** = Rule Checker, **R** = Notification Service, WebSocket Service
...
| `ws-positions` | WebSocket Service | gps-events |
| `ws-events` | WebSocket Service | geozone-events, rule-violations |
